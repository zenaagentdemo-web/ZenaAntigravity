        ...
        {loading ? (
          ...
          displayActions.map((action, index) => {
            ...
            return (
              <button
                key={action.id}
                className={`quick-action-button quick-action-button--${action.color} ${
                  isVoiceNoteActive ? 'quick-action-button--active' : ''
                } ${isActionLoading ? 'quick-action-button--loading' : ''} ${
                  focusedActionId === action.id ? 'quick-action-button--focused' : ''
                }`}
                onClick={() => handleActionClick(action)}
                disabled={isActionDisabled}
                onFocus={() => setFocusedActionId(action.id)}
                onBlur={() => setFocusedActionId(null)}
                aria-label={`${action.label}${action.shortcut ? `. Keyboard shortcut: Alt plus ${action.shortcut}` : ''}${
